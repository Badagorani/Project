...
namespace OMRON_Camera_Control
{
	class CameraControl
	{
        ...
        public CameraControl(MainForm form)
		{
			...
			{
				//mainform.ShowMessage("오류", "제공된 매개변수가 유효하지 않거나 범위를 벗어났습니다!\n" + ex.Message, "경고");
				mainform.ShowMessage("오류", "카메라의 연결 상태를 확인해주세요!\n" + Log.CameraErrorCode(ex.Message), "경고");
				Log.LogWrite($"{this.GetType().Name} -> {MethodBase.GetCurrentMethod().Name} " + Log.CameraErrorCode(ex.Message));
			}
        }
        ...
        private void open()
		{
			...
			{
				mainform.ShowMessage("오류", "카메라 오픈에 실패하였습니다!\n" + Log.CameraErrorCode(ex.Message), "경고");
				Log.LogWrite($"{this.GetType().Name} -> {MethodBase.GetCurrentMethod().Name} " + Log.CameraErrorCode(ex.Message));
			}
        }
        public void Close()
		{
            ...
            catch (Exception ex)
            {
				...
				mainform.ShowMessage("종료", "카메라의 상태를 확인해주세요!\n" + Log.CameraErrorCode(ex.Message), "경고");
				Log.LogWrite($"{this.GetType().Name} -> {MethodBase.GetCurrentMethod().Name} " + Log.CameraErrorCode(ex.Message));
			}
        }
        public void Start()
		{
			...
			{
				mainform.ShowMessage("오류", "시작 실패 : 카메라의 상태를 확인해주세요!\n" + Log.CameraErrorCode(ex.Message), "경고");
				Log.LogWrite($"{this.GetType().Name} -> {MethodBase.GetCurrentMethod().Name} " + Log.CameraErrorCode(ex.Message));
			}
        }
        public void Stop()
        ...
        public void SetEnableImageCallback(bool value)
		{
            ...
            if (m_Camera_DataStream == null)
            {
				...
				mainform.ShowMessage("오류", "데이터 스트림을 만드는데 실패했습니다!", "주의");
                return;
			}
            if (value == true)
            {
        ...
        private void OnCallback(IStCallbackParamBase paramBase, object[] param)
		{
            ...
            else
            {
				...
				mainform.ShowMessage("오류", "잘못된 CallBack 수신입니다!\n" + strError, "주의");
				Log.LogWrite($"{this.GetType().Name} -> {MethodBase.GetCurrentMethod().Name} " + strError);
			}
        }
        public byte[] ColorBuffer
        {
			get
			{
				...
				if (m_Camera_device != null)
				{
                    ...
                    {
						mainform.ShowMessage("오류", "잘못된 ColorBuffer 참조입니다!\n" + Log.CameraErrorCode(ex.Message), "주의");
						Log.LogWrite($"{this.GetType().Name} -> {MethodBase.GetCurrentMethod().Name} " + Log.CameraErrorCode(ex.Message));
						return m_Camera_lastColorBuffer.GetIStImage().GetByteArray();
                    }
                }
        ...
        public void SetIP(string ipvalue)
		{
            ...
            try
			{
                ...
                if (iInterface == null)
                {
					mainform.ShowMessage("오류", "인터페이스를 읽어오는데 실패했습니다!\n카메라의 IP를 바꿀 수 없습니다!\n", "주의");
                    return;
                }
                ...
                {
					mainform.ShowMessage("오류", "잘못된 IP 주소입니다!\n", "주의");
                    return;
                }
                ...
                {
					mainform.ShowMessage("오류", "잘못된 IP입니다!", "주의");
                    return;
                }
            ...
            {
				mainform.ShowMessage("오류", "IP를 변경하던 중 오류가 발생했습니다!\n" + Log.CameraErrorCode(ex.Message), "주의");
				Log.LogWrite($"{this.GetType().Name} -> {MethodBase.GetCurrentMethod().Name} " + Log.CameraErrorCode(ex.Message));
				return;
            }
        }
